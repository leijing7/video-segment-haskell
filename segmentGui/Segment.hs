...
segmentVideo [clipInfo] basename offsetTime = do 
	...
	let avCmd = "ffmpeg\\ffmpeg -i " ++ basename ++ ".avi -ss " ++ startTime ++ " -t " 
				++ duration ++ " " ++ basename ++ "/" ++syllable ++ ".avi"
	let audioOnlyCmd = "ffmpeg\\ffmpeg -i " ++ basename ++ ".avi -vn -ss " ++ startTime ++ " -t " 
				++ duration ++ " " ++ basename ++ "_audioOnly/" ++syllable ++ ".wav"
	let videoOnlyCmd = "ffmpeg\\ffmpeg -i " ++ basename ++ ".avi -an -ss " ++ startTime ++ " -t " 
				++ duration ++ " " ++ basename ++ "_videoOnly/" ++syllable ++ ".avi"
	system avCmd
	system audioOnlyCmd
	system videoOnlyCmd
	--print avCmd
...
main = do
	...
	let videoPaths = [x|x<-filesInDir, L.isInfixOf ".avi" x]
